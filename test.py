...
TRAIN_DIR = '/Users/jintpongchababnapa/Documents/Dissertation/PPO-Irrigation-main/train_output'
EVAL_DIR = './test_output'
WEATHER_PATH = '/Users/jintpongchababnapa/Documents/Dissertation/PPO-Irrigation-main/weather_data/champion_climate.txt'
...
def irrigation_efficiency(yield_tonnes, irrigation_mm):
    if irrigation_mm == 0:
        return 0.00
    yield_kg = yield_tonnes * 1000
    return yield_kg / irrigation_mm
...
def setup_env(seed):
    def _init():
        set_seed(seed)
        env = Monitor(Wheat(sim_start=2008, sim_end=2018))
        env.reset()
        return env
...
def evaluate(agent, env, episodes=EVAL_EPISODES):
    ...
    for _ in range(episodes):
        ...
        results.append({
            ...
            'reward': reward_sum,
            'efficiency': irrigation_efficiency(info.get('dry_yield', 0), info.get('total_irrigation', 0))
        })
...
def evaluate_heuristics():
    ...
    for name, strategy in strategies.items():
        ...
        results.append({
            ...
            'water_efficiency': irrigation_efficiency(
...
def load_model(seed, timestep):
    set_seed(seed)
    env = setup_env(seed)
    norm_path = os.path.join(TRAIN_DIR, f"ppo_{timestep}_norm.pkl")
    if os.path.exists(norm_path):
        env = VecNormalize.load(norm_path, env)
        env.training = False
        env.norm_reward = False
    model_path = os.path.join(TRAIN_DIR, f"ppo_{timestep}")
    model = IrrigationPPO.load(model_path, env=env)
    return model, env
